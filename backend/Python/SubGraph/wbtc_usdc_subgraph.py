...
def get_swaps():
    ...
    # Ethereum mainnet WBTC/USDC pool address (0.05% fee tier)
...
def fetch_wbtc_usdc_data():
    ...
    try:
        ...
        if swaps:
            ...
            return {
                "error": "Failed to fetch WBTC/USDC swaps data",
                "swaps": [],
                "total_swaps": 0
            }
    except Exception as e:
        return {
            "error": f"Error in fetch_wbtc_usdc_data: {str(e)}",
            "swaps": [],
            "total_swaps": 0
        }
...
def main():
    print("Testing WBTC/USDC SubGraph with provided API key...")
    
    # Test the main fetch function
    data = fetch_wbtc_usdc_data()
    if "error" in data:
