...
from SubGraph.weth_usdc import get_swaps, get_pool_data
...
def extract_prices_from_swaps(swaps_data, pool_data):
    ...
    For WETH/USDC, we want WETH price in USD.
    """
    prices = []
    ...
    for swap in reversed(swaps_data):  # Reverse to get chronological order
        try:
            ...
            # For WETH/USDC pool, calculate WETH price from swap amounts
            if token0_symbol == 'USDC' and token1_symbol == 'WETH':
                # USDC is token0, WETH is token1
                if amount1 != 0:
                    # Price = USD amount / WETH amount
                    price = amount_usd / abs(amount1)
            elif token0_symbol == 'WETH' and token1_symbol == 'USDC':
            ...
            if price is None or price <= 0:
                if token0_symbol == 'USDC' and token1_symbol == 'WETH' and amount0 != 0 and amount1 != 0:
                    # USDC amount / WETH amount = WETH price in USDC
                    price = abs(amount0) / abs(amount1)
                elif token0_symbol == 'WETH' and token1_symbol == 'USDC' and amount0 != 0 and amount1 != 0:
                    # USDC amount / WETH amount = WETH price in USDC
                    price = abs(amount1) / abs(amount0)
            
            if price and price > 0 and price < 1000000:  # Sanity check for reasonable WETH price
                prices.append(price)
...
def get_formatted_data():
    """
    Fetch and format data for WETH/USDC pair
    """
    try:
...
def health_check():
    ...
    return jsonify({"status": "healthy", "pair": "WETH/USDC"})
