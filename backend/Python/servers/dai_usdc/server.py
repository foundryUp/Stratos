...
from SubGraph.dai_usdc import get_swaps, get_pool_data
...
def extract_prices_from_swaps(swaps_data, pool_data):
    ...
    For DAI/USDC, we want DAI price in USD (should be close to 1.0).
    """
    prices = []
    ...
    for swap in reversed(swaps_data):  # Reverse to get chronological order
        try:
            ...
            # For DAI/USDC pool, calculate DAI price from swap amounts
            if token0_symbol == 'USDC' and token1_symbol == 'DAI':
                # USDC is token0, DAI is token1
                if amount1 != 0:
                    # Price = USD amount / DAI amount
                    price = amount_usd / abs(amount1)
            elif token0_symbol == 'DAI' and token1_symbol == 'USDC':
                # DAI is token0, USDC is token1
                if amount0 != 0:
                    # Price = USD amount / DAI amount  
            ...
            if price is None or price <= 0:
                if token0_symbol == 'USDC' and token1_symbol == 'DAI' and amount0 != 0 and amount1 != 0:
                    # USDC amount / DAI amount = DAI price in USDC
                    price = abs(amount0) / abs(amount1)
                elif token0_symbol == 'DAI' and token1_symbol == 'USDC' and amount0 != 0 and amount1 != 0:
                    # USDC amount / DAI amount = DAI price in USDC
                    price = abs(amount1) / abs(amount0)
            
            if price and price > 0.5 and price < 2.0:  # Sanity check for reasonable DAI price (should be ~$1)
                prices.append(price)
                print(f"Extracted price: ${price:.4f}")
...
def get_formatted_data():
    """
    Fetch and format data for DAI/USDC pair
    """
    try:
...
def health_check():
    ...
    return jsonify({"status": "healthy", "pair": "DAI/USDC"})
