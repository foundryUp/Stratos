...
contract AaveV3InteractorTest is Test {
    ...
    function setUp() public {
        ...
        string memory arbRpcUrl = vm.envString("ARB_RPC_URL");
    ...
    function testDepositBorrowRepayWithdraw() public {
        // Define amounts.
        uint256 depositAmountUSDC = 10000 * 1e6; // 10,000 USDC (USDC has 6 decimals)
        uint256 borrowAmountWETH = 1 * 1e18;       // 1 WETH (WETH has 18 decimals)
        ...
        interactor.deposit(USDC, depositAmountUSDC);

        // Borrow 1 WETH from Aave using the deposited collateral.
        interactor.borrow(WETH, borrowAmountWETH, 2);
