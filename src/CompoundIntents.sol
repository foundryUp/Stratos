...
contract CompoundIntents {
    ...
    function _deposit(uint256 amount) internal {
        ...
        (bool success, ) = compoundManager.call{value: amount}(abi.encodeWithSignature("depositETH()"));
        require(success, "Deposit failed.");
        cEth.transfer(msg.sender, cEth.balanceOf(address(this)));
    }

    function _withdraw(uint256 amount) internal {
        (bool success, ) = compoundManager.call(abi.encodeWithSignature("withdrawETH(uint256)", amount));
