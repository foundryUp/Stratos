...
contract AaveV3Interactor {
    ...
    function borrow(address asset, uint256 amount, uint256 interestRateMode) external onlyOwner {
    ...
    function withdraw(address asset, uint256 amount) external onlyOwner {
        uint256 withdrawnAmount = pool.withdraw(asset, amount, address(this));
        // Transfer the withdrawn tokens back to the owner.
        require(IERC20(asset).transfer(msg.sender, withdrawnAmount), "Transfer failed");
    }
}
