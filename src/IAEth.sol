...
contract IETH {
    ...
    address internal constant CURIA =
        0x0000000000001d8a2e7bf6bc369525A2654aa298;

    /// @dev The Escrows protocol singleton.
    address internal constant ESCROWS =
        0x00000000000044992CB97CB1A57A32e271C04c11;
        ...
        )
    {
        ...
        if (
            ...
            (to, amount, token, callData, executeCallData) = _previewSend(
                _to,
                _amount,
                _token
            );
            ...
            address _receiver;
            (amount, minAmountOut, token, to, _receiver) = _previewSwap(
                amountIn,
                amountOutMin,
                tokenIn,
                tokenOut,
            ...
            action == "lock" || action == "lockup" || action == "escrow"
        ) {
            (
                bytes memory _to,
                bytes memory _amount,
            ...
            (to, amount, token, minAmountOut /*expiry*/) = _previewLock(
                _to,
                _amount,
                _token,
                _time,
        ...
        )
    {
        ...
        _amount = _toUint(
            amount,
            decimals != 0 ? decimals : _token.readDecimals(),
            _token
        );
        ...
        )
    {
        ...
        _amountIn = _toUint(
            amountIn,
            decimalsIn != 0 ? decimalsIn : _tokenIn.readDecimals(),
            _tokenIn
        );
        _amountOut = _toUint(
            amountOutMin,
        ...
        if (receiver.length != 0)
            (, _receiver, ) = whatIsTheAddressOf(string(receiver));
    }
    ...
    ) public view virtual returns (bool intentMatched) {
    ...
    function _returnTokenConstants(
        bytes32 token
    ) internal pure virtual returns (address _token, uint256 _decimals) {
        ...
        if (token == "wbtc" || token == "btc" || token == "bitcoin")
            return (WBTC, 8);
        if (token == "steth" || token == "wsteth" || token == "lido")
            return (WSTETH, 18);
        if (token == "reth") return (RETH, 18);
    }

    /// @dev Checks and returns the canonical token string constant for a matched address.
    function _returnTokenAliasConstants(
        address token
    ) internal pure virtual returns (string memory _token, uint256 _decimals) {
        if (token == USDC) return ("USDC", 6);
    ...
    function _returnPoolConstants(
        ...
        if (token0 == WETH && token1 == USDT)
            return 0x11b815efB8f581194ae79006d24E0d814B7697F6;
        if (token0 == DAI && token1 == WETH)
            return 0xC2e9F25Be6257c210d7Adf0D4Cd6E3E881ba25f8;
    ...
    function _escrow(
        ...
        if (_token == ETH) {
            unchecked {
                require(msg.value == _amount);
                return
                    IEscrows(ESCROWS).escrow{value: _amount}(
                        address(0),
    ...
    function swap(
        ...
        uint256 decimalsIn;
        uint256 decimalsOut;
        if (bytes(tokenIn).length == 42)
            info.tokenIn = _toAddress(bytes(tokenIn));
        else
            (info.tokenIn, decimalsIn) = _returnTokenConstants(
                bytes32(bytes(tokenIn))
            );
        if (info.tokenIn == address(0)) info.tokenIn = addresses[tokenIn];
        if (bytes(tokenOut).length == 42)
            info.tokenOut = _toAddress(bytes(tokenOut));
        else
            (info.tokenOut, decimalsOut) = _returnTokenConstants(
                bytes32(bytes(tokenOut))
            );
    ...
    function order(
        ...
        if (bytes(tokenIn).length == 42) o.tokenIn = _toAddress(bytes(tokenIn));
            ...
            (o.tokenIn, decimalsIn) = _returnTokenConstants(
                bytes32(bytes(tokenIn))
            );
        if (o.tokenIn == address(0)) o.tokenIn = addresses[string(tokenIn)];
        if (bytes(tokenOut).length == 42)
            o.tokenOut = _toAddress(bytes(tokenOut));
        else
            (o.tokenOut, decimalsOut) = _returnTokenConstants(
                bytes32(bytes(tokenOut))
            );
        if (o.tokenOut == address(0))
            o.tokenOut = addresses[string(bytes(tokenOut))];

        o.amountIn = _toUint(
            bytes(amountIn),
            decimalsIn != 0 ? decimalsIn : o.tokenIn.readDecimals(),
            o.tokenIn
        );
        o.amountOut = _toUint(
            bytes(amountOut),
            decimalsOut != 0 ? decimalsOut : o.tokenOut.readDecimals(),
            o.tokenOut
        );
    ...
    ) public pure virtual returns (string memory intent) {
    ...
    function translateExecute(
        bytes calldata callData
    ) public view virtual returns (string memory intent) {
        unchecked {
            ...
            bool transfer = bytes4(callData[132:136]) ==
                IToken.transfer.selector;

            (
                string memory token,
                uint256 decimals
            ) = _returnTokenAliasConstants(target);
    ...
    function translateUserOp(
        PackedUserOperation calldata userOp
    ) public view virtual returns (string memory intent) {
        return
            bytes4(userOp.callData) == IExecutor.execute.selector
    ...
    ) internal view virtual returns (bytes32 node) {
    ...
    function _split(
        bytes memory base,
        bytes1 delimiter
    ) internal pure virtual returns (StringPart[] memory parts) {
        unchecked {
    ...
    function _toAddress(
        bytes memory s
    ) internal pure virtual returns (address addr) {
        unchecked {
    ...
    function _convertWeiToString(
        ...
        unchecked {
            ...
            return
                string(abi.encodePacked(wholeNumberStr, ".", decimalPartStr));
        }
    }

    /// @dev Remove any trailing zeroes from bytes.
    function _removeTrailingZeros(
        bytes memory str
    ) internal pure virtual returns (string memory) {
        unchecked {
